...
$query = 'from(bucket: "Donnees_SolarEdge")
    |> range(start: -20y)
    |> filter(fn: (r) => r["_measurement"] == "puissance_jour")
    ...
    foreach ($result as $table) {
        foreach ($table->records as $record) {
            $puissance_jour[] = [
                ...
                'puissance_jour' => $record->getValue()
            ];
        }
    }
} catch (Exception $e) {
...
<head>
    ...
    <title>Mesures de puissance par jour</title>
...
<body>
    ...
    <main>
        <h1>Mesures des 3 derniers jours</h1>
        <table>
            <thead>
                <tr>
                    <th>Heure</th>
            ...
            <tbody>
                <?php foreach ($puissance_jour as $row): ?>
                <tr>
                    <td><?= date("Y-m-d H:i:s", strtotime($row['heure'])) ?></td>
                    <td><?= $row['puissance_jour'] ?> W</td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
