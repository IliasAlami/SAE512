...
$query = 'from(bucket: "Donnees_SolarEdge")
    |> range(start: -20y)
    |> filter(fn: (r) => r["_measurement"] == "puissance")
...
<head>
    ...
    <title>Mesures de puissance</title>
...
<body>
    ...
    <main>
        <h1>Dernière Mesure de puissance</h1>
        <table>
            <thead>
                <tr>
                    <th>Heure</th>
            ...
            <tbody>
                <?php foreach ($puissance as $row): ?>
                <tr>
                    <td><?= date("Y-m-d H:i:s", strtotime($row['heure'])) ?></td>
                    <td><?= $row['puissance'] ?> W</td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
